            ...
            <form action="{{ route('orders.store') }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2">
                        <x-input-label for="description" :value="__('Diagnóstico Inicial / Motivo de Entrada')" />
                        <textarea id="description" name="description" class="mt-1 block w-full bg-slate-900 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all p-4" rows="4" required placeholder="Ej: Ruido en la suspensión delantera, cambio de aceite..."></textarea>
                ...
                <div class="flex items-center justify-end mt-8 gap-4 border-t border-slate-700 pt-8">
                    <a href="{{ route('orders.index') }}" class="text-sm font-bold text-slate-400 hover:text-white transition-colors">Cancelar</a>
