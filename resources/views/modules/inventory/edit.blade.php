            ...
            <form action="{{ route('inventory.update', $part) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $part->name)" required autofocus />
                        <x-input-error class="mt-2" :messages="$errors->get('name')" />
                    </div>

                    <div>
                        <x-input-label for="sku" :value="__('SKU / Código')" />
                        <x-text-input id="sku" name="sku" type="text" class="mt-1 block w-full" :value="old('sku', $part->sku)" />
                        <x-input-error class="mt-2" :messages="$errors->get('sku')" />
                    </div>

                    <div>
                        <x-input-label for="category" :value="__('Categoría')" />
                        <x-text-input id="category" name="category" type="text" class="mt-1 block w-full" :value="old('category', $part->category)" />
                        <x-input-error class="mt-2" :messages="$errors->get('category')" />
                    </div>

                    <div>
                        <x-input-label for="stock" :value="__('Stock Actual')" />
                        <x-text-input id="stock" name="stock" type="number" step="0.01" class="mt-1 block w-full" :value="old('stock', $part->stock)" required />
                        <x-input-error class="mt-2" :messages="$errors->get('stock')" />
                    </div>

                    <div>
                        <x-input-label for="min_stock" :value="__('Stock Mínimo (Alerta)')" />
                        <x-text-input id="min_stock" name="min_stock" type="number" step="0.01" class="mt-1 block w-full" :value="old('min_stock', $part->min_stock)" required />
                        <x-input-error class="mt-2" :messages="$errors->get('min_stock')" />
                    </div>

                    <div class="md:col-span-2">
                        <x-input-label for="price" :value="__('Precio de Venta ($)')" />
                        <x-text-input id="price" name="price" type="number" step="0.01" class="mt-1 block w-full" :value="old('price', $part->price)" required />
